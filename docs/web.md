...
- **RegisterApiPage**: 3단계 폼. 제출 시 `POST /api/products` → `isActive=false` 저장 → 관리자 승인 대기. SuccessStep에 "관리자 승인 후 게시" 문구.
- **ApiDetailPage**: `spec_json`을 SwaggerUI로 렌더링. `@JsonRawValue` 덕분에 파싱 가능.
