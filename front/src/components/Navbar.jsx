...
export default function Navbar() {
  ...
  const tierColors = {
    FREE: 'text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800',
    PRO: 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30',
    ENTERPRISE: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30',
  }
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-1.5 w-56 bg-white dark:bg-gray-900 rounded-xl shadow-lg shadow-black/10 dark:shadow-black/40 border border-gray-200 dark:border-gray-700 py-1.5 z-50">
                    <div className="px-4 py-2.5 border-b border-gray-100 dark:border-gray-800">
                      ...
                      <span className={`inline-block mt-1.5 px-2 py-0.5 text-xs font-medium rounded-md ${tierColors[user?.tier] || tierColors.FREE}`}>
                        {user?.tier} 플랜
                      </span>
                    </div>
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-4 py-2 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors mt-0.5"
                    >
                      로그아웃
                    </button>
                  </div>
