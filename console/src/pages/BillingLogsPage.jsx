...
export default function BillingLogsPage() {
  ...
  useEffect(() => {
    setLoading(true)
    client.get('/admin/usage/logs', { params: { onlyErrors, page, size: PAGE_SIZE } })
      .then(({ data }) => {
        setItems(data.items)
        setTotal(data.total)
      })
              ...
              {loading ? (
                ...
                items.map((log) => (
                  <tr key={log.id} className="border-b border-gray-100 dark:border-gray-800 last:border-0">
                    <td className="px-5 py-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">{log.requestTime?.replace('T', ' ')}</td>
